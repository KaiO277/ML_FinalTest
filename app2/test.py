...
def show_new_page(): 
    ...
    new_window.geometry("800x500")  # Adjusted window size

    left_frame = ttk.Frame(new_window, padding="20")
    left_frame.grid(row=0, column=0, sticky="nsew")

    right_frame = ttk.Frame(new_window, padding="20")
    ...
    if(model=="Linear Regression"):
        ...
        label = ttk.Label(right_frame, text=f"Root Mean Squared Error (RMSE): {rmse:.2f}")
        label.pack(pady=20)
    ...
    if(model=="Logistic Regression"):
        ...
        label = ttk.Label(right_frame, text=f"Độ chính xác: {accuracy:.2f}")
        label.pack(pady=20)
